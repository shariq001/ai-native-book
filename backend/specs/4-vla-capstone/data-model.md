# Data Model: Vision-Language-Action (VLA) & Capstone Educational Module
...
- **Title**: "Vision-Language-Action (VLA) & Capstone Educational Module" (string, required)
- **Description**: Educational content explaining the convergence of Language Models with physical robotic actions (string, required)
- **TargetAudience**: "Students ready for system integration" (string, required)
...
- **ArchitecturePattern**: The VLA pipeline architecture (string, required)
...
- **ConceptsCovered**: List of VLA concepts covered in the chapter (array of strings, required)
- **Prerequisites**: List of prerequisite concepts required for understanding (array of strings, required)
- **RelatedModules**: IDs of related modules that provide foundational knowledge (array of strings, optional)
...
- **SectionFlow**: The position in the sequential flow (Embodied LLM → Robot Ear → Cognitive Planner → Capstone) (string, required)

### VisualElement
- **ID**: Unique identifier for the visual element (string, required)
- **ChapterID**: Reference to parent chapter (string, required)
- **Type**: Category of visual ("2D Block Diagram", "SVG Flowchart", "VLA Pipeline", "Prompt Template") (string, required)
- **Content**: The actual diagram/chart content (string, required)
...
- **Topic**: The specific topic the visual relates to (e.g., "VLA Pipeline", "Prompt Template") (string, required)
- **FileSizeKB**: Size of the visual asset in KB (integer, required)

### Student
- **ID**: Unique identifier for the student (string, required)
- **DisplayName**: Name to display to the student (string, optional)
- **Background**: Technical background ("AI", "Robotics", etc.) (string, optional)
...
- Chapters array must contain exactly 4 chapter IDs in correct sequence
- Learning objectives must contain at least 3 items
- Duration must be between 60 and 180 minutes
- ArchitecturePattern must be "VLA Pipeline: Microphone → Whisper → LLM → ROS 2"
...
- SectionFlow must be one of: "Embodied LLM", "Robot Ear", "Cognitive Planner", "Capstone"
...
- Content must not exceed 50KB (51200 bytes)
- FileSizeKB must be <= 50
